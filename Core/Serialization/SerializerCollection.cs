...
{
    /// <summary>
    /// A enum dictating how the collection will handle duplicate serializers in <see cref="SerializerCollection.AddSerializer"/>
    /// </summary>
            ...
            if (_serializers.ContainsKey(serializableType))
            {
                switch (duplicateImplementationResolution)
                {
                    ...
                    case DuplicateImplementationResolution.Skip:
                        break;
                    default:
                        throw new ArgumentOutOfRangeException(nameof(duplicateImplementationResolution), duplicateImplementationResolution, null);
                }
    ...
    /// <summary>
    /// Scans the assembly for <see cref="IStreamSerializerFor{T}"/> implementations with no <see cref="FileVersionAttribute"/> and registers them to the collections.
    /// Types already registered will silently be skipped. Useful for complementing a set of tagged serializers with compatible default serializers
    /// </summary>
    ...
    /// <summary>
    /// Scans the assembly for <see cref="IStreamSerializerFor{T}"/> implementations that has a <see cref="FileVersionAttribute"/> and registers them to the collections.
    /// If a found type is already registered, it will throw 
    /// </summary>
