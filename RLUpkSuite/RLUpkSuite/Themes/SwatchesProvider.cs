...
namespace RLUpkSuite.Themes;

/// <summary>
///     Provides access to all colour swatches.  For information regarding Material Design colours see
...
public class SwatchesProvider
{
    ...
    public SwatchesProvider(Assembly assembly)
    {
        ...
        ResourceManager manager = new(resourcesName, assembly);
        var resourceSet = manager.GetResourceSet(CultureInfo.CurrentUICulture, true, true);
        var dictionaryEntries = resourceSet?.OfType<DictionaryEntry>().ToList();
        var assemblyName = assembly.GetName().Name;

        Regex regex = new(@"^themes\/materialdesigncolor\.(?<name>[a-z]+)\.(?<type>primary|secondary)\.baml$");
    ...
    private static Swatch CreateSwatch(string name, ResourceDictionary? primaryDictionary,
        ResourceDictionary? secondaryDictionary)
    {
        ...
        static Hue GetHue(ResourceDictionary dictionary, DictionaryEntry entry)
        {
            ...
            var foregroundKey = entry.Key + "Foreground";
    ...
    {
        if (assemblyName is null || path is null) return null;
